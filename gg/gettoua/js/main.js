$(function () {
	...
	let minitranslateWidth = [];
	let minislideNow = [];


	let minislideCount = [];
	for (let i = 1; i < $('.prev-btn').length; i++) {
		minitranslateWidth[i] = 0;
		minislideNow[i] = 0;
		minislideCount[i] = $('#mini-slidewrapper' + i).children().length;
	}
	console.log(minitranslateWidth + " trans" + minislideNow + " slide" + minislideCount + "   slidecount");
	console.log($('#mini-slidewrapper' + 1).length);


	function miniNextSlide(i) {
		console.log(minislideCount[i] + " " + minislideNow[i] + "MINI" + "     " + minitranslateWidth[i]);

		if (minislideNow[i] >= minislideCount[i] - 1 || minislideNow[i] < 0) {
			$('#mini-slidewrapper' + i).css('transform', 'translate(0, 0)');
			minislideNow[i] = 0;
		} else {
			minitranslateWidth[i] = -$('#mini-viewport' + i).width() * (minislideNow[i]);
			console.log(minitranslateWidth[i]);
			$('#mini-slidewrapper' + i).css({
				'transform': 'translate(' + minitranslateWidth[i] + 'px, 0)',
				'-webkit-transform': 'translate(' + minitranslateWidth[i] + 'px, 0)',
				'-ms-transform': 'translate(' + minitranslateWidth[i] + 'px, 0)',
			});
			minislideNow[i]++;
		}
	}

	function miniPrevSlide(i) {
		if (minislideNow[i] == 0 || minislideNow[i] <= 0 || minislideNow[i] > minislideCount[i] - 1) {
			minitranslateWidth[i] = -$('#mini-viewport' + i).width() * (minislideCount[i] - 1);
			...
			minislideNow[i] = minislideCount[i] - 1;
		} else {
			minitranslateWidth[i] = -$('#mini-viewport' + i).width() * (minislideNow[i] - 2);
	...
	$('#mini-next-btn1').click(function () {
		miniNextSlide(1);
	});

	$('#mini-prev-btn1').click(function () {
		miniPrevSlide(1);
	});


	$('#mini-next-btn2').click(function () {
		miniNextSlide(2);
	});

	$('#mini-prev-btn2').click(function () {
		miniPrevSlide(2);
	});


	$('#mini-next-btn3').click(function () {
		miniNextSlide(3);
	});

	$('#mini-prev-btn3').click(function () {
		miniPrevSlide(3);
	});


	$('#mini-next-btn4').click(function () {
		miniNextSlide(4);
	});

	$('#mini-prev-btn4').click(function () {
		miniPrevSlide(4);
	});


	$('#mini-next-btn5').click(function () {
		miniNextSlide(5);
	});

	$('#mini-prev-btn5').click(function () {
		miniPrevSlide(5);
	});

	$('#mini-next-btn6').click(function () {
		miniNextSlide(6);
	});

	$('#mini-prev-btn6').click(function () {
		miniPrevSlide(6);
	});

	$('#mini-next-btn7').click(function () {
		miniNextSlide(7);
	});

	$('#mini-prev-btn7').click(function () {
		miniPrevSlide(7);
	});

	$('#mini-next-btn8').click(function () {
		miniNextSlide(8);
	});

	$('#mini-prev-btn8').click(function () {
		miniPrevSlide(8);
	});

	$('#mini-next-btn9').click(function () {
		miniNextSlide(9);
	});

	$('#mini-prev-btn9').click(function () {
		miniPrevSlide(9);
	});
	...
	$(".search_item-location input").on("input", (e) => {
		...
		$(".search_item-location input").on("input", (e) => {
			...
			request.addEventListener("load", function () {
				...
				console.log(data); // смотрим ответ сервера



				if (search.val().length > 0) {
					$('.location-result  li').remove();
					for (let i = 0; i < data.length; i++) {

						$('.location-result ul').append(
							'<li>' +

							'<img src="./img/geo.svg" alt="">' +
							'<div class="location-result-title">' +
							+(data[i].city) +
							'<div class="location-result-subtitle">' +
							'<p class="transparent">-</p>' + (data[i].country) +
							'</div>' +
							'</div>' +
	...
	$("#slider-range").slider({
		...
		slide: function (event, ui) {
			...
			$("#rangeFromValue").val(ui.values[0]);
			$("#rangeToValue").val(ui.values[1]);
		}
	});
	$("#amount").val("$" + $("#slider-range").slider("values", 0) +
		" - $" + $("#slider-range").slider("values", 1));

	$("#rangeFromValue").val($("#slider-range").slider("values", 0));
	$("#rangeToValue").val($("#slider-range").slider("values", 1));
